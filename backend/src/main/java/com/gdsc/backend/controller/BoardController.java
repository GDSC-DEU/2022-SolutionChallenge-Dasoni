...
public class BoardController {
    ...
    public Page<Board> getProjects(@Parameter(name = "boardCategory", in = ParameterIn.PATH, description = "조회할 지원사업의 카테고리") @PathVariable("boardCategory") BoardCategory boardCategory) {
       return this.boardService.getCategory(boardCategory);
    }

    @GetMapping(value = "/All", produces = MediaType.APPLICATION_JSON_VALUE)
    public Page<Board> getAllProjects(@PageableDefault Pageable pageable){
        return this.boardService.findProjects(pageable);
    }

    @GetMapping(value = "/Bookmark", produces = MediaType.APPLICATION_JSON_VALUE)
    public Page<Board> getBookmark(@PageableDefault Pageable pageable){
        return this.boardService.getBookmark(pageable);
    }
